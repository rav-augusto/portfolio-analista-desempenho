...
export default function DashboardAtletasPage() {
  ...
  // Projeção de temporada (seletor de nº de jogos)
  ...
  // Agregado de todas as avaliações (com posição) p/ comparativo por posição
  ...
  const handleExportarDossie = () => {
    ...
    const ok = abrirDossieParaImpressao({
      atleta: {
        ...
        altura: atletaAtual.altura,
        peso: atletaAtual.peso,
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-100">Dashboard de Atletas</h1>
          <p className="text-sm text-slate-400 mt-1">Acompanhe a evolução dos atletas</p>
        </div>
        <Link
            ...
            <div className="rounded-2xl p-4 md:p-6 shadow-sm mb-4 md:mb-6" style={{ backgroundColor: '#1e293b', border: '1px solid #475569' }}>
              <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6">
                ...
                <div className="flex-1 text-center sm:text-left">
                  <h2 className="text-xl md:text-2xl font-bold text-slate-100">{atletaAtual.nome}</h2>
                ...
                <div className="rounded-2xl p-4 md:p-6 shadow-sm" style={{ backgroundColor: '#1e293b', border: '1px solid #475569' }}>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-base md:text-lg font-semibold text-slate-100">Evolução</h3>
                      <span className="text-xs text-slate-500 hidden sm:inline">(1ª vs última)</span>
                    </div>
                ...
                <div className="rounded-2xl p-4 md:p-6 shadow-sm mb-4 md:mb-6" style={{ backgroundColor: '#1e293b', border: '1px solid #475569' }}>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-base md:text-lg font-semibold text-slate-100">Estatísticas de Jogo</h3>
                        <p className="text-xs text-slate-400 hidden sm:block">Minutagem, gols e assistências</p>
                      </div>
                    </div>
                  ...
                  {/* Linha de médias (por jogo / por 90 min) */}
                  {minutosData.total > 0 && (
                    <div className="flex flex-wrap items-center justify-center sm:justify-end gap-x-4 gap-y-1 mb-4 -mt-1 text-[11px] md:text-xs text-slate-400">
                      <span className="flex items-center gap-1">
                        <span className="text-slate-500">⌀ Min/jogo:</span>
                        <span className="font-semibold text-amber-400">{mediasJogo.minutosPorJogo.toFixed(0)}&apos;</span>
                      </span>
                      <span className="flex items-center gap-1">
                        <span className="text-slate-500">G+A/jogo:</span>
                        <span className="font-semibold text-violet-300">{mediasJogo.participacoesPorJogo.toFixed(2)}</span>
                      </span>
                      <span className="flex items-center gap-1">
                        <span className="text-slate-500">Gols/partida (60&apos;):</span>
                        <span className="font-semibold text-green-400">{mediasJogo.golsPorPartida.toFixed(2)}</span>
                      </span>
                      <span className="flex items-center gap-1">
                        <span className="text-slate-500">G+A/partida (60&apos;):</span>
                        <span className="font-semibold text-violet-300">{mediasJogo.participacoesPorPartida.toFixed(2)}</span>
                      </span>
                    </div>
                  )}

                  {/* Insights ofensivos (regularidade, sequências, ritmo) */}
                  {insightsOfensivos.totalJogos > 0 && (
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3 mb-4">
                      ...
                      {/* Sequência atual */}
                      <div className="rounded-xl p-2.5 md:p-3" style={{ backgroundColor: '#0f172a', border: '1px solid #475569' }}>
                        <p className="text-[9px] md:text-[10px] text-slate-400 uppercase mb-1">Sequência atual</p>
                        <p className="text-lg md:text-xl font-black text-orange-400">
                          {insightsOfensivos.sequenciaAtual > 0 ? `🔥 ${insightsOfensivos.sequenciaAtual}` : '—'}
                        </p>
                        <p className="text-[9px] md:text-[10px] text-slate-500">{insightsOfensivos.sequenciaAtual > 0 ? 'jogos seguidos com participação' : 'sem participação no último jogo'}</p>
                      </div>
                      {/* Melhor sequência */}
                      <div className="rounded-xl p-2.5 md:p-3" style={{ backgroundColor: '#0f172a', border: '1px solid #475569' }}>
                        <p className="text-[9px] md:text-[10px] text-slate-400 uppercase mb-1">Melhor sequência</p>
                      ...
                      <div className="rounded-xl p-2.5 md:p-3" style={{ backgroundColor: '#0f172a', border: '1px solid #475569' }}>
                        ...
                        <p className="text-lg md:text-xl font-black text-green-400">
                          {insightsOfensivos.minutosPorGol > 0 ? `${insightsOfensivos.minutosPorGol.toFixed(0)}'` : '—'}
                        </p>
                  ...
                  {/* Gráfico de Participações Acumuladas (trajetória de crescimento) */}
                          ...
                          options={{
                            ...
                            plugins: {
                              legend: {
                                display: true,
                                position: 'top' as const,
                                labels: { color: '#e2e8f0', boxWidth: 12, padding: 8, font: { size: 10 } }
                              },
                              tooltip: {
                                backgroundColor: '#0f172a',
                                titleColor: '#a855f7',
                    ...
                    <div className="mt-4 rounded-xl p-3" style={{ backgroundColor: '#0f172a', border: '1px solid #475569' }}>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="text-xs md:text-sm font-medium text-slate-200">
                          Comparativo com a posição — <span className="text-cyan-400">{comparativoPosicao.posicao}</span>
                            ...
                            <div key={m.label} className="flex items-center gap-2 md:gap-3">
                              ...
                              <div className="flex-1 h-5 rounded-full relative overflow-hidden" style={{ backgroundColor: '#1e293b' }}>
                                <div
                                  ...
                                  style={{ width: `${(m.atleta / escala) * 100}%`, backgroundColor: acima ? '#22c55e' : '#ef4444', opacity: 0.85 }}
                                />
                                {/* marcador da média da posição */}
                                <div
                                  className="absolute top-0 bottom-0"
                                  style={{ left: `${(m.media / escala) * 100}%`, width: 2, backgroundColor: '#e2e8f0' }}
                                  title={`Média da posição: ${m.media.toFixed(2)}`}
                                />
                      ...
                      <p className="text-[10px] text-slate-500 mt-2 flex items-center gap-1">
                        <span className="inline-block" style={{ width: 8, height: 8, backgroundColor: '#e2e8f0' }} /> linha branca = média da posição
                      </p>
                    </div>
                  )}
              ...
              {avaliacoes.length > 0 && (
                <div className="rounded-2xl p-4 md:p-6 shadow-sm" style={{ backgroundColor: '#1e293b', border: '1px solid #475569' }}>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 md:gap-4 mb-4">
                    <h3 className="text-base md:text-lg font-semibold text-slate-100">Detalhes da Avaliação</h3>
                    <div className="flex items-center gap-2">
